...
CompSurfContext::CompSurfContext(const char *accessToken,
    ...
    initVulkan(nativeWindow, width, height);
}
...
void CompSurfContext::quit(int rc) {
    if (gVulkanDriver.device && vkDeviceWaitIdle) {
        vkDeviceWaitIdle(gVulkanDriver.device);
    }
    destroySwapchainThings(true);
...
void CompSurfContext::createInstance() {
...
void CompSurfContext::createSurface(void *native_window) {
    typedef struct _wl {
        struct wl_display *display;
        struct wl_surface *surface;
    } wl;

    wl* ptrval = reinterpret_cast<wl*>(native_window);
    ...
    VkResult result = vkCreateWaylandSurfaceKHR(gVulkanDriver.instance, &createInfo, NULL, &gVulkanDriver.surface);
    if (result != VK_SUCCESS) {
        gVulkanDriver.surface = VK_NULL_HANDLE;
        utils::slog.e << "vkCreateWaylandSurfaceKHR(): "
                      << SDL_GetError() << utils::io::endl;
        quit(2);
    }
}

void CompSurfContext::findPhysicalDevice() {
...
void CompSurfContext::createDevice() {
...
void CompSurfContext::getQueues() {
...
void CompSurfContext::createSemaphore(VkSemaphore *semaphore) {
    VkResult result;
...
void CompSurfContext::getSurfaceFormats() {
...
void CompSurfContext::getSwapchainImages() {
    VkResult result;
...
bool CompSurfContext::createSwapchain(int w, int h) {
    uint32_t i;
...
void CompSurfContext::destroySwapchain() {
...
void CompSurfContext::destroyCommandBuffers() {
...
void CompSurfContext::destroyCommandPool() {
...
void CompSurfContext::createCommandPool() {
    VkResult result;
...
void CompSurfContext::createCommandBuffers() {
    VkResult result;
...
void CompSurfContext::createFences() {
    uint32_t i;
...
void CompSurfContext::destroyFences() {
    uint32_t i;
...
void CompSurfContext::recordPipelineImageBarrier(VkCommandBuffer commandBuffer,
...
void CompSurfContext::rerecordCommandBuffer(uint32_t frameIndex, const VkClearColorValue *clearColor) {
...
void CompSurfContext::destroySwapchainThings(bool doDestroySwapchain) {
    destroyFences();
...
bool CompSurfContext::createNewSwapchainThings(int w, int h) {
    destroySwapchainThings(false);
    getSurfaceCaps();
    getSurfaceFormats();
...
void CompSurfContext::initVulkan(void *native_window, int w, int h) {
    ...
    getQueues();
    createSemaphores();
    createNewSwapchainThings(w, h);
}

bool CompSurfContext::render() {
    const float SPEED = 4;
    uint32_t frameIndex;
    VkResult result;
    double currentTime;
    ...
    if (!gVulkanDriver.swapchain) {
        bool retval = createNewSwapchainThings(mWidth, mHeight);
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        return createNewSwapchainThings(mWidth, mHeight);
    }
